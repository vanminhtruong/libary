...
import { logout } from '@/layout/service/logout.js';
...
const handleLogout = async () => {
    confirm.require({
        message: t('orderapproval.logout.confirm_message'),
        header: t('orderapproval.logout.confirm_header'),
        icon: 'pi pi-exclamation-triangle text-yellow-500',
        acceptLabel: t('orderapproval.logout.accept'),
        rejectLabel: t('orderapproval.logout.reject'),
        accept: async () => {
            try {
                ...
                if (resp.status === 200) {
                    toast.add({
                        severity: 'success',
                        summary: t('orderapproval.logout.success_summary'),
                        detail: t('orderapproval.logout.success_detail'),
                        life: 3000
                    });
                    setTimeout(() => {
                        router.push('/login');
                ...
                toast.add({
                    severity: 'error',
                    summary: t('orderapproval.logout.error_summary'),
                    detail: t('orderapproval.logout.error_detail'),
                    life: 3000
                });
            }
        }
    ...
    <div class="layout-topbar">
        ...
        <div class="layout-topbar-actions">
            ...
            <div class="flex justify-center ml-4 items-center">
                <svg class="cursor-pointer" @click="handleLogout" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#EA3323">
